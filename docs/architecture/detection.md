...
`duel_end`). v1.1.4 also accepts `frame_base64` so the Plugin can submit a
captured PNG frame without writing fixture files. It returns per-template
`name`, `kind`, `score`, and `matched` fields with diagnostics such as
`template_match_missing` or `templates_not_configured`.

---

## Matching

The original v0.8 path uses deterministic local byte matching:
...
v1.1.4 adds bounded local PNG image matching for templates generated by the
setup workflow:

- templates and frames remain local runtime data under `user_data/`
- PNG frames are submitted with `frame_base64`
- 8-bit non-interlaced grayscale, RGB, and RGBA PNG inputs are supported
- score is based on the best pixel-similarity match between the template image
  and the submitted frame image
- exact matches return `1.0`; lower-confidence matches can be accepted by
  lowering the template threshold

The Worker still owns image matching. The OBS Plugin should capture and
transport the current frame, but it must not perform heavy image processing.
This keeps the same API boundary open for future OpenCV/Pillow-backed matching
without changing Dock UI ownership.

The periodic automatic feed uses the transport contract documented in
`docs/architecture/automatic-frame-feed.md`. The Plugin sends captured PNG
frames to `POST /detection/frame` as `frame_base64`; the Worker performs all
